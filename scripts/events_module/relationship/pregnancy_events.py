...
def handle_zero_moon_pregnant(cat: Cat, other_cat: Optional[Cat] = None):
    ...
    if get_clan_setting("same sex birth"):
        ...
        create_pregnancy_data(pregnant_cat, second_parent)
        handle_pregnancy_event(pregnant_cat, second_parent)
        return
...
def handle_pregnancy_event(pregnant_cat, second_parent):
    ...
    if second_parent and second_parent.ID in pregnant_cat.mate:
        text, involved_cats = create_pregnancy_announcement(
    ...
    # sometimes they won't...
        ...
        set_affair_visibility_on_pregnancy(
...
def handle_two_moon_pregnant(cat: Cat):
    ...
    if other_cat and cat.mate and other_cat.ID not in cat.mate:
        ...
        if cheated_mate:
            ...
            if not affair_known and random.randint(0, 1):
                ...
                mate_claimed_kits = should_claim_affair_kits(cheated_mate, cat)
    ...
    involved_cats = remove_unmentioned_mate_ids(involved_cats, print_event, cat_dict)
    ...
    if cheated_mate and not mate_claimed_kits and not secret_affair_birth:
        handle_affair_discovery_breakup(cat, cheated_mate)
        ...
        if other_cat and other_cat.mate:
            ...
            if other_cat_mate and other_cat_affair_known:
                handle_affair_discovery_breakup(other_cat, other_cat_mate)
...
def _handle_main_birth_event(
    ...
    # SAME-SEX RELATIONSHIP AFFAIR
        ...
        involved_cats.append(other_cat.ID)
        cat_dict["r_c"] = other_cat
        chosen_mate = get_cheated_mate(cat)
        if chosen_mate:
    ...
    # MAIN CAT CHEATED ON MATE
        ...
        living_mate = get_cheated_mate(cat)
        dead_mate = get_cheated_mate(cat, include_dead=True)
        involved_cats.append(other_cat.ID)
        cat_dict["r_c"] = other_cat
        if living_mate:
    ...
    # OTHER CAT CHEATED ON MATE
        ...
        other_mate = get_cheated_mate(other_cat)
        if other_mate:
...
def get_second_parent(cat: Cat) -> tuple[Optional[Cat], bool]:
    ...
    new_partner = determine_highest_romantic_relation(
...
def determine_highest_romantic_relation(
    ...
    if mate and highest_romantic_relation:
        ...
        love_affair_chance = get_love_affair_chance(
    ...
    # COPARENTING
        ...
        coparenting_chance = get_unmated_coparenting_chance(highest_romantic_relation)
